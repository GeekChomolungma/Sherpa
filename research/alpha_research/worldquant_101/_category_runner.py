...
def run_category(
    ...
    *,
    ...
    """对 `alpha_classes` 里每个 alpha：先跑第一层 `alpha_check` 过滤，通过的才跑第二层
    `run_vectorized_backtest`。占位因子（`compute()` 直接 `raise NotImplementedError`）
    如实打印"跳过"，不是 bug——世坤101里 19 个因子本来就因为缺行业分类/市值数据算不出来。
